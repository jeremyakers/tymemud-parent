#!/usr/bin/env python3
"""Test transaction with debug output."""
...
class TransactionContext:
    ...
    async def __aenter__(self):
        ...
        cmd = f"tx_begin ZONES {zone_str}"
        print(f"[DEBUG TX] Sending: {cmd}")
        await self.client._send(cmd)

        response = ""
        for i in range(3):
            response = await self.client._read_line()
            print(f"[DEBUG TX] tx_begin response {i}: {response!r}")
            if response:
                break
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        for i in range(3):
            response = await self.client._read_line()
            print(f"[DEBUG TX] {cmd} response {i}: {response!r}")
            if response:
                break
...
async def main():
    ...
    try:
        async with client.transaction([0]) as tx:
            print(f"Transaction started! active={tx.active}")
            print("Exiting transaction (should commit)...")
